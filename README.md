...
Beyond the core booking flow, the app includes **cancel** (frees bay + technician), a searchable **day agenda**, bay **utilization**, technician filtering, suggested free times, a **⌘K** command palette, and keyboard slot navigation.

Full requirements are in [`docs/questions`](docs/questions). Architecture and API detail: [`docs/specs/2026-07-15-unified-service-scheduler-system-design.md`](docs/specs/2026-07-15-unified-service-scheduler-system-design.md).
...
- **`apps/web`** — Vite + React 19 SPA. Fake SSO, booking wizard, day board, agenda, cancel/details popover, and command palette. Server state lives in TanStack Query; motion handles transitions.
- **`apps/api`** — FastAPI modular monolith with async SQLAlchemy. Availability search, transactional booking, soft-cancel, structured JSON logging with request IDs, Alembic migrations.
- **PostgreSQL** — source of truth for dealership resources and appointments (active bookings enforced by a partial unique index on bay + start).
...
Start Postgres first (from the repo root). The Compose `db` service publishes **port 5432** — use that in `DATABASE_URL` unless you have a reason to map a different host port.
...
$env:DATABASE_URL = "postgresql+asyncpg://scheduler:scheduler@localhost:5432/scheduler"   # PowerShell
# or
export DATABASE_URL=postgresql+asyncpg://scheduler:scheduler@localhost:5432/scheduler   # bash
...
> If port 5432 is already in use on your machine, either stop the conflicting service or remap the container (e.g. `-p 5433:5432` in a custom `docker run`) and update `DATABASE_URL` to match.
...
2. **Design spec** — turned the brainstorm into a written design document (`docs/specs/2026-07-15-unified-service-scheduler-system-design.md`) covering architecture, domain model, API surface, and explicit out-of-scope items, so the "what" and "why" were fixed before implementation started.
3. **Implementation plan** — decomposed the spec into a numbered, file-scoped task list (`docs/plans/2026-07-15-unified-service-scheduler.md`), each task ending in an explicit verification step and a commit boundary.
...
- **Cancel is supported** (soft-cancel → `cancelled`); **reschedule**, multi-dealership, WebSockets/SSE, soft holds, and payments remain out of scope.
